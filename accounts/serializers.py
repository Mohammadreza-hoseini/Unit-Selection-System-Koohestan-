...
class StudentGetDataSerializer(serializers.ModelSerializer):
    ...
    faculty_detail = FacultyGetDataSerializer(source='faculty')
    major_detail = MajorGetDataSerializer(source='major')
    supervisor_detail = ProfessorGetDataSerializer(source='supervisor')

    class Meta:
        model = Student
        fields = (
            'id', 'firstname', 'lastname', 'student_number', 'email', 'phone', 'national_code', 'gender', 'birth_date',
            'entry_year', 'incoming_semester', 'average', 'term_detail', 'faculty_detail', 'major_detail', 'passed_lessons',
            'lessons_in_progress',
            'supervisor_detail', 'military_service_status', 'years',)
